...
def songs():
    ...
    if request.method == "POST":
        ...
        search_response = requests.get("https://api.spotify.com/v1/search", params=params, headers=headers,verify=False)

        if search_response.status_code == 200:
            search_data = search_response.json()
            tracks = search_data.get('tracks', {}).get('items', [])

            return render_template('songs.html', query=query,tracks=tracks)
        else:
...
def song(id):
    ...
    track_response = requests.get("https://api.spotify.com/v1/tracks/" + id, params=params, headers=headers,
                                  verify=False)
    ...
    if request.method == "POST":
        cursor.execute('SELECT * FROM user_' + str(session["user"]["id"]) + 'event_' + str(session["selected_event"]) + ' WHERE id = ?', (id,))
        thisSong = cursor.fetchone()

        cursor.execute('SELECT * FROM event_songs_' + str(session["selected_event"]) + ' WHERE id = ?', (id,))
        event_song = cursor.fetchone()
        ...
        if thisSong:
            cursor.execute('DELETE FROM user_' + str(session["user"]["id"]) + 'event_' + str(session["selected_event"]) + ' WHERE id = ?', (id,))
            cursor.execute('INSERT OR REPLACE INTO event_songs_' + str(session["selected_event"]) + ' (id, name, image, votes) VALUES (?, ?, ?, ?)',(id, track_data["name"],track_data['album']['images'][0]['url'],votes-1))
        else:
            cursor.execute('INSERT INTO user_' + str(session["user"]["id"]) + 'event_' + str(session["selected_event"]) + ' VALUES (?)', (id,))
            cursor.execute('INSERT OR REPLACE INTO event_songs_' + str(session["selected_event"]) + ' (id, name, image, votes) VALUES (?, ?, ?, ?)',(id, track_data["name"],track_data['album']['images'][0]['url'],votes+1))

        db.commit()

    cursor.execute('SELECT * FROM user_' + str(session["user"]["id"]) + 'event_' + str(
        session["selected_event"]) + ' WHERE id = ?', (id,))
    thisSong = cursor.fetchone()
    ...
    return render_template('song.html', data=track_data, voted=voted)
